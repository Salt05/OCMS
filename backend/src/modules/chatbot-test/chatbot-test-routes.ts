...
export const chatbotTestRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  // All test routes require authenticated user with admin/owner role
  app.addHook('preHandler', authMiddleware);
  app.addHook('preHandler', async (req: any, reply) => {
    if (!['owner', 'admin'].includes(req.user?.role)) {
      return reply.status(403).send({ error: 'Forbidden: Chỉ quản trị viên mới có quyền truy cập Chatbot Test' });
    }
  });
  ...
  app.post('/api/v1/chatbot-test/conversations/:id/simulate', async (req: any, reply) => {
    try {
      const orgId = req.user.orgId;
      const { id } = req.params;
      const { content = '', senderName, imageUrl } = req.body || {};

      if (!content?.trim() && !imageUrl?.trim()) {
        return reply.status(400).send({ error: 'Nội dung tin nhắn hoặc hình ảnh không được để trống' });
      }
      ...
      const isImg = !!imageUrl?.trim();
      const contentType = isImg ? 'image' : 'text';
      const attachments = isImg ? [{ url: imageUrl.trim(), type: 'image' }] : [];
      const msgContent = content?.trim() || imageUrl.trim();
  ...
  // ── 3.1. Upload Image for Test Conversation (Customer simulation or Staff takeover) ──
  app.post('/api/v1/chatbot-test/conversations/:id/upload', async (req: any, reply) => {
    try {
      const orgId = req.user.orgId;
      const { id } = req.params;
      ...
      if (!data) {
        return reply.status(400).send({ error: 'Chưa chọn file hình ảnh' });
      }

      const role = (data.fields?.role as any)?.value || 'customer';
      const caption = (data.fields?.caption as any)?.value || '';

      const originalName = data.filename || `image_${Date.now()}.jpg`;
      const ext = path.extname(originalName).toLowerCase() || '.jpg';
      const filename = `test_${Date.now()}_${randomUUID().slice(0, 8)}${ext}`;
      ...
      const message = await prisma.message.create({
        data: {
          ...
          conversationId: conversation.id,
          zaloMsgId: null,
          senderType: isCustomer ? 'contact' : 'self',
          senderUid: isCustomer ? (conversation.contact?.zaloUid || 'test_customer') : req.user.id,
          senderName: isCustomer ? (conversation.contact?.fullName || 'Khách Hàng') : (req.user.fullName || req.user.email || 'Nhân Viên'),
          content: caption ? `${caption}\n${fileUrl}` : fileUrl,
          contentType: 'image',
          attachments: [{ url: fileUrl, type: 'image', name: originalName }],
          isNote: false,
          isAi: false,
          sentAt: new Date(),
        },
      ...
      await prisma.conversation.update({
        ...
        data: {
          lastMessageAt: message.sentAt,
          unreadCount: isCustomer ? { increment: 1 } : 0,
          isReplied: !isCustomer,
        },
      });
      ...
      if (isCustomer) {
        chatbotService
          ...
          .catch((e) => {
            logger.error('[chatbot-test] AI pipeline error with image:', e.message);
          });
      }
      ...
      logger.error('[chatbot-test] Upload image error:', err.message);
      return reply.status(500).send({ error: err.message });
    }
  });
