  ...
  <div class="chatbot-test-view h-100 w-100 d-flex" style="overflow: hidden; max-width: 100%;">
    ...
    <div class="chat-main flex-grow-1 d-flex flex-column h-100" style="min-width: 0; flex: 1 1 0%; overflow: hidden;">
        ...
        <div class="d-flex align-center gap-3">
          ...
          <div>
            <div class="d-flex align-center gap-2">
              ...
              <v-chip v-if="selectedConv.contact?.salutation" size="x-small" color="purple" variant="flat" class="font-weight-bold">
                Xưng hô: {{ selectedConv.contact.salutation }}
              </v-chip>
        ...
        >
          ...
          <div v-if="msg.senderType === 'contact'" class="d-flex align-start gap-2 max-w-75">
            ...
            <div>
              ...
              <div v-if="getImageUrl(msg)" class="mb-1">
                <a :href="getImageUrl(msg)!" target="_blank" rel="noopener noreferrer">
                  <img
                    :src="getImageUrl(msg)!"
                    alt="Ảnh gửi từ khách"
                    class="test-chat-image"
                  />
                </a>
              </div>
              <div v-if="msg.content && msg.content !== getImageUrl(msg)" class="customer-bubble pa-3 rounded-xl rounded-ts-0 text-body-2">
                {{ msg.content }}
              </div>
          ...
          <div v-else class="d-flex align-start gap-2 max-w-75 flex-row-reverse">
            ...
            <div class="text-right">
              ...
              <div v-if="getImageUrl(msg)" class="mb-1 d-flex justify-end">
                <a :href="getImageUrl(msg)!" target="_blank" rel="noopener noreferrer">
                  <img
                    :src="getImageUrl(msg)!"
                    alt="Ảnh đính kèm"
              ...
              <div v-if="msg.content && msg.content !== getImageUrl(msg)" class="bot-bubble pa-3 rounded-xl rounded-te-0 text-body-2 text-left">
                {{ msg.content }}
              </div>
              <div class="text-caption text-disabled mt-1" style="font-size: 10px">
                {{ formatTime(msg.sentAt) }}
      ...
      <div v-if="selectedConv" class="chat-input-area pa-3 border-t bg-surface flex-shrink-0">
        ...
        <div v-if="selectedImagePreview" class="d-flex align-center gap-2 mb-2 pa-2 bg-surface-light rounded-lg border">
          <img :src="selectedImagePreview" class="rounded" style="width: 44px; height: 44px; object-fit: cover;" />
          <div class="flex-grow-1 text-truncate">
            <div class="text-caption font-weight-medium text-truncate">{{ selectedImageFile?.name || 'Ảnh đính kèm' }}</div>
            <div class="text-caption text-medium-emphasis" style="font-size: 10px">
              {{ (selectedImageFile?.size ? (selectedImageFile.size / 1024).toFixed(1) + ' KB' : '') }} • Sẵn sàng gửi kèm
            </div>
          </div>
          <v-btn icon="mdi-close" size="x-small" variant="text" color="error" @click="clearSelectedImage" title="Hủy ảnh" />
        </div>
        ...
        <div class="d-flex align-center gap-2">
          ...
          <v-textarea
            ...
            :placeholder="sendRole === 'customer' ? 'Nhập câu hỏi hoặc dán hình ảnh Ctrl+V (Enter để gửi)...' : 'Nhập câu trả lời hoặc dán hình ảnh...'"
            variant="outlined"
            density="compact"
            rows="1"
            max-rows="4"
          ...
          <v-btn
            ...
            icon="mdi-send"
            :loading="sendingMsg"
            :disabled="!inputContent.trim() && !selectedImageFile"
            @click="handleSend"
          />
        </div>
      </div>
...
import { useChatbotTest, type TestConversation, type TestMessage } from '@/composables/use-chatbot-test';
...
function getImageUrl(msg: TestMessage): string | null {
  if (msg.attachments) {
    ...
    if (Array.isArray(atts) && atts.length > 0) {
      const img = atts.find(
        (a: any) => a.url && (a.type?.includes('image') || /\.(jpg|jpeg|png|webp|gif)$/i.test(a.url)),
      );
      if (img?.url) return img.url;
      if (atts[0]?.url) return atts[0].url;
    }
  }
  ...
  if (msg.content?.startsWith('{')) {
    try {
      ...
      if (href && (/\.(jpg|jpeg|png|webp|gif)/i.test(href) || href.includes('zdn.vn'))) return href;
    } catch {}
  }
  return null;
}
...
async function handleSend() {
  ...
  try {
    if (file) {
      await uploadTestImage(file, text || undefined, sendRole.value);
    } else {
