<template>
  <v-dialog :model-value="modelValue" max-width="960" scrollable @update:model-value="$emit('update:modelValue', $event)">
    <v-card class="order-detail-card rounded-lg">
          ...
          <div class="mb-4">
            <div class="d-flex align-center justify-space-between mb-2">
              <span class="text-subtitle-2 font-weight-bold d-flex align-center gap-2">
                ...
                Danh sách sản phẩm ({{ order.lines?.length || 0 }})
              </span>
            </div>
          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-card variant="outlined" class="h-100 rounded-lg pa-4">
                ...
                <div v-if="cleanNote(order.note)" class="text-body-2 text-medium-emphasis order-note-content" v-html="cleanNote(order.note)" />
                <div v-else class="text-body-2 text-medium-emphasis font-italic">Không có ghi chú thêm</div>
              </v-card>
            </v-col>
      ...
      <v-card-actions class="px-6 py-3 bg-surface">
        <v-spacer />
