    ...
    <v-card class="rounded-xl overflow-hidden d-flex flex-column" style="width: 880px; max-width: 95vw; height: 680px; max-height: 90vh;">
      ...
      <div class="d-flex flex-grow-1 overflow-hidden" style="min-height: 420px;">
        ...
        <div class="d-flex flex-column flex-grow-1 border-e overflow-hidden" style="width: 58%;">
          ...
          <!-- Tabs: Bạn bè | Nhóm trò chuyện + Phân loại dropdown -->
          <div class="d-flex align-center justify-space-between px-4 border-b flex-shrink-0">
            <v-tabs
              v-model="activeTab"
              density="compact"
              color="primary"
              class="bulk-picker-tabs"
            >
              <v-tab value="friends" class="text-none font-weight-medium" style="font-size: 0.85rem;">
                Bạn bè
              </v-tab>
              <v-tab value="groups" class="text-none font-weight-medium" style="font-size: 0.85rem;">
                Nhóm trò chuyện
              </v-tab>
            </v-tabs>
          ...
          <!-- Contact items scroll list: STRICTLY Name + Avatar (No extra info) -->
          <div class="flex-grow-1 overflow-y-auto px-2 py-1">
            ...
            >
              ...
              <!-- Name ONLY (No subtitle, no customer code clutter) -->
...
// Filtered list based on active tab, search query, and tag filter
...
async function loadData() {
  ...
  try {
    const [convRes, contactsRes] = await Promise.allSettled([
      api.get('/conversations', { params: { limit: 100 } }),
    ...
    if (contactsRes.status === 'fulfilled' && contactsRes.value.data?.contacts) {
      contactsList.value = contactsRes.value.data.contacts.map((ct: any) => ({
        id: ct.id,
        contactId: ct.id,
        conversationId: ct.conversations?.[0]?.id || null,
