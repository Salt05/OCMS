    ...
    async def send_message(
        ...
        *,
        ...
        try:
            ...
            if "429" in err_msg_lower or "rate limit" in err_msg_lower or "rate_limit" in err_msg_lower:
                user_friendly_error = "⚠️ AI đang tạm thời vượt quá giới hạn lượt gọi trong 1 phút (Rate Limit 429). Vui lòng đợi 3 - 5 giây và gửi lại câu hỏi nhé!"
            else:
                user_friendly_error = f"Lỗi xử lý tin nhắn: {str(e)}"
    ...
    async def _send_message(
        ...
        *,
        ...
        if request_context and request_context.metadata:
            conversation.metadata.update(request_context.metadata)
