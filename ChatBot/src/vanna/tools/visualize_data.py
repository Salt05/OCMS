...
class VisualizeDataArgs(BaseModel):
    ...
    chart_type: Optional[str] = Field(
        default=None,
        description="Loại biểu đồ cần vẽ: 'pie' (biểu đồ tròn/donut cho tỷ lệ, cơ cấu, top N phần trăm), 'bar' (biểu đồ cột), 'line' (biểu đồ đường theo thời gian), 'scatter' (phân tán), hoặc 'auto' (tự động chọn phù hợp nhất)"
    )
...
class VisualizeDataTool(Tool[VisualizeDataArgs]):
    ...
    def description(self) -> str:
        return "Tạo biểu đồ trực quan (cột, tròn/pie, đường) từ file CSV kết quả truy vấn. Hãy chỉ định chart_type='pie' nếu người dùng muốn biểu đồ tròn/tỷ lệ hoặc chart_type='bar' cho biểu đồ cột."
    ...
    async def execute(
        ...
        try:
            logger.info(f"Starting visualization for file: {args.filename}, type: {args.chart_type}")
