...
class OpenAILlmService(LlmService):
    ...
    async def send_request(self, request: LlmRequest) -> LlmResponse:
        ...
        # Retry with backoff for rate limits (429) or transient errors
        resp = None
        max_retries = 4
        for attempt in range(max_retries):
            try:
                ...
                if is_rate_limit and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 0.8
                    ms_match = re.search(r"try again in ([\d\.]+)s", err_str)
                    if ms_match:
                        try:
                            wait_time = float(ms_match.group(1)) + 0.2
                        except Exception:
                            pass
                    else:
                        ms_match_ms = re.search(r"try again in ([\d\.]+)ms", err_str)
                        if ms_match_ms:
                            try:
                                wait_time = (float(ms_match_ms.group(1)) / 1000.0) + 0.3
                            except Exception:
                                pass
                    print(f"[RateLimit] 429 hit in send_request. Retrying attempt {attempt + 1}/{max_retries} after {wait_time:.2f}s...")
                    ...
                    if ms_match:
                        ...
                        if ms_match_ms:
                            try:
                                ...
                                pass
                    print(f"[RateLimit] 429 hit in stream_request. Retrying attempt {attempt + 1}/{max_retries} after {wait_time:.2f}s...")
                    await asyncio.sleep(max(wait_time, 0.5))
                    continue
                raise e
    ...
    def _build_payload(self, request: LlmRequest) -> Dict[str, Any]:
        ...
        temp = request.temperature if hasattr(request, "temperature") and request.temperature is not None else self.temperature
        if temp is not None:
